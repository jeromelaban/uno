...
namespace Uno.UI.Runtime.Skia
{
	public partial class GtkCoreWindowExtension : ICoreWindowExtension
	{
		...
		private void OnKeyPressEvent(object sender, KeyPressEventArgs args)
		{
			try
			{
				var virtualKey = ConvertKey(args.Event.Key);

				if (this.Log().IsEnabled(LogLevel.Trace))
				{
					this.Log().Trace($"OnKeyPressEvent: {args.Event.Key} -> {virtualKey}");
				}
				...
				_ownerEvents.RaiseKeyDown(
					new KeyEventArgs(
						...
						{
							ScanCode = args.Event.HardwareKeycode,
							RepeatCount = 1,
						}));
			}
			catch (Exception e)
			{
				...
				{
					this.Log().Error("Failed to raise KeyDown", e);
				}
			}
		}
		...
		private void OnKeyReleaseEvent(object sender, KeyReleaseEventArgs args)
		{
			try
			{
				...
				if (this.Log().IsEnabled(LogLevel.Trace))
				{
					this.Log().Trace($"OnKeyReleaseEvent: {args.Event.Key} -> {virtualKey}");
				}
			...
			{
				this.Log().Error("Failed to raise KeyUp", e);
			}
		}
