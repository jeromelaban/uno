	...
	{	
		private void ReloadFile(FileReload fileReload)
		{
			if (Environment.GetEnvironmentVariable("DOTNET_MODIFIABLE_ASSEMBLIES") == "debug")
			{
				...
				{
					this.Log().LogError($".NET Hot Reload is enabled, skipping XAML Reader reload");
				}
				return;
			}
			...
			foreach (var updatedType in updatedTypes)
			{
				...
				if (updatedType.Is<UIElement>())
				{
					foreach (var instance in EnumerateInstances(Window.Current.Content, i => updatedType.IsInstanceOfType(i)))
					{
						...
						{
							typeof(ClientHotReloadProcessor).Log().LogWarning($"Creating instance of type {instance.GetType()}");
						...
						{
							typeof(ClientHotReloadProcessor).Log().LogWarning($"Type [{updatedType}] has no parameterless constructor, skipping");
						}
					}
				}
				else
				{
					typeof(ClientHotReloadProcessor).Log().LogWarning($"Type [{updatedType}] is not a UIElement, skipping");
				}
			}
		}
