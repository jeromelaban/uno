...
namespace SamplesApp.UITests.Runtime
{
	...
	public partial class RuntimeTests : SampleControlUITestBase
	{
		...
		public async Task RunRuntimeTests()
		{
			...
			{
				return await GetWithRetry("IsTestExecutionDone", () => unitTestsControl.GetDependencyPropertyValue("RunningState")?.ToString().Equals("Finished", StringComparison.OrdinalIgnoreCase) ?? false);
			}
			...
			{
				var newValue = await GetWithRetry("GetRunTestCount", () => unitTestsControl.GetDependencyPropertyValue("RunTestCount")?.ToString());
			...
			var count = GetValue("FailedTestCount", unitTestsControl);
			if (count != "0")
			{
