...
namespace Uno.UI.DataBinding
{
	...
	internal class BindingPath : IDisposable, IValueChangedListener
	{
		...
		private sealed class BindingItem : IBindingItem, IDisposable
		{
			...
			private void OnPropertChanged(object? previousValue, object? newValue, bool shouldRaiseValueChanged)
			{
			...
			private IDisposable SubscribeToPropertyChanged()
			{
				...
				for (var i = 0; i < _propertyChangedHandlers.Count; i++)
				{
					...
					Action? updateProperty = () =>
					{
						...
						OnPropertChanged(previousValue, newValue, shouldRaiseValueChanged: true);
					...
					if (handlerDisposable != null)
					{
						...
						disposables.Add(() =>
						{
							...
							OnPropertChanged(previousValue, DependencyProperty.UnsetValue, shouldRaiseValueChanged: false);
						});
					}
				}
