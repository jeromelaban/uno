...
#pragma warning restore 649 // Unused member
			...
			catch (Exception e)
			{
				...
				foreach (var exception in Flatten(e))
				{
					var diagnostic = Diagnostic.Create(
						...
						GetLocation(exception),
						exception.Message + " " + exception.GetType()
						+ " " + (exception is XmlException ex ? $"sourceUri: {ex.SourceUri}" : "<null>")
						+ " " + (exception is XamlParseException xamlEx ? $"LineNumber: {xamlEx.LineNumber}" : "<null>")
					);
		...
		private Location? GetLocation(Exception exception)
		{
			if (exception is XamlParsingException xamlParsingException)
			{
					...
					&& xamlParsingException.LinePosition.HasValue)
				{
					return Location.Create(
						xamlFile.Path,
						xamlText.Lines[xamlParsingException.LineNumber.Value-1].Span,
						new LinePositionSpan(
							new LinePosition(xamlParsingException.LineNumber.Value-1, xamlParsingException.LinePosition.Value-1),
							new LinePosition(xamlParsingException.LineNumber.Value-1, xamlParsingException.LinePosition.Value-1)
						)
					);
				}
